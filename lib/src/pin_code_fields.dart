...
class PinCodeTextField extends StatefulWidget {
  ...
  final TextStyle? obscuringCharacterTextStyle;
  ...
  PinCodeTextField({
    ...
    this.obscuringCharacterTextStyle,
...
class _PinCodeTextFieldState extends State<PinCodeTextField>
  ...
  Widget _renderPinField({
    ...
    final text = _pinTheme.shape == PinCodeFieldShape.none
    ...
    return widget.textGradient != null
        ...
        : Text(
            text,
            key: ValueKey(_inputList[index]),
            style: _pinTheme.shape == PinCodeFieldShape.none
                ? (text == "●"
                    ? widget.obscuringCharacterTextStyle ??
                        TextStyle(
                          fontSize: 20,
                          fontWeight: FontWeight.bold,
                        )
...
// selects the right fill color for the field
  ...
  List<Widget> _generateFields() {
    ...
    for (int i = 0; i < widget.length; i++) {
      late Border boder = Border();
      if (_pinTheme.shape == PinCodeFieldShape.none) {
        boder = Border();
      } else {
        if (_pinTheme.shape == PinCodeFieldShape.underline) {
          boder = Border(
            bottom: BorderSide(
          ...
          boder = Border.all(
              color: _getColorFromIndex(i), width: _pinTheme.borderWidth);
        }
      }
      result.add(
        Container(
